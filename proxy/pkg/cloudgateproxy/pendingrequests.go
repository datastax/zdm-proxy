...
func (p *pendingRequests) timeOut(streamId int16, reqCtx RequestContext, nodeMetrics *metrics.NodeMetrics, req *frame.RawFrame) bool {
	holder := p.getOrCreateRequestContextHolder(streamId)
	if reqCtx.SetTimeout(nodeMetrics, req) {
...
func (p *pendingRequests) markAsDone(
	streamId int16, nodeMetrics *metrics.NodeMetrics,
	f *frame.RawFrame, cluster ClusterType, connectorType ClusterConnectorType) (RequestContext, bool) {
	holder := p.getOrCreateRequestContextHolder(streamId)
	reqCtx := holder.Get()
	if reqCtx == nil {
	...
	if reqCtx.SetResponse(nodeMetrics, f, cluster, connectorType) {
		var err error
